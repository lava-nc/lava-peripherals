...
class DummySwipeDetector():
    def __init__(self,
                 send_pipe: type(Pipe),
                 num_steps: int,
                 use_loihi2: bool,
                 dummy_input_config: ty.Optional[dict] = None,
        ...
        self.dummy_input_config = dummy_input_config or dummy_input_default_config
        ...
        self.num_neurons = np.prod(self.dummy_input_config["size"])
        _, _, self.scaled_height, self.scaled_width = self.dummy_input_config["size"]
    ...
    def create_weights(self):
        ...
        # Left weights
        self.rec_weights_left = np.zeros((self.num_neurons, self.num_neurons)).reshape(
            (self.scaled_height, self.scaled_width, self.scaled_height, self.scaled_width))
        ...
        # right weights
        self.rec_weights_right = np.zeros((self.num_neurons, self.num_neurons)).reshape(
            (self.scaled_height, self.scaled_width, self.scaled_height, self.scaled_width))
        for i in range(self.scaled_height):
            for j in range(self.scaled_width):
    ...
    def create_processes(self):
        ...
        input_data = np.ones((self.num_neurons, self.num_steps))
        self.frame_input = RingBuffer(data=input_data)

        print("Create inp processes")
        self.ff_inp = Sparse(weights=self.ff_weights, num_message_bits=8)
        self.lif_inp = LIF(shape=self.dummy_input_config["size"], **self.lif_config)

        print("Create left processes")
        self.ff_left = Sparse(weights=self.ff_weights)
        self.rec_left = Sparse(weights=self.rec_weights_left)
        self.lif_left = LIF(shape=self.dummy_input_config["size"], **self.lif_config)
        ...
        self.lif_right = LIF(shape=self.dummy_input_config["size"], **self.lif_config)
        ...
        self.recv = ProcessOut(send_pipe=self.send_pipe,
            shape=self.dummy_input_config["size"],
    ...
    def make_connections(self):
        ...
        self.frame_input.s_out.reshape(self.dummy_input_config["size"]).connect(self.recv.frame_in)
