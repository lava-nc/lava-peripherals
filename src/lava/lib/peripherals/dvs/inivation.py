...
class InivationDATRecorder(AbstractProcess):
    """
    Process that receives events from Inivation device and sends them out as a histogram. 
...
@implements(proc=InivationDATRecorder, protocol=LoihiProtocol)
@requires(CPU)
@tag('floating_pt')
class PyInivationDATRecorderModel(PyLoihiProcessModel):
    s_out: PyOutPort = LavaPyType(PyOutPort.VEC_DENSE, np.int32)
    ...
    def __init__(self, proc_params):
        ...
        self.fn = "recording.dat"
        self.dat_writer = DatWriter(self.fn, height=self.height, width=self.width)
    ...
    def run_spk(self):
        ...
        if events is not None:
            events_np = events.numpy()
            events = np.zeros(len(events_np), dtype=np.dtype([("y", int), ("x", int), ("p", int), ("t", np.int64)]))
            events['t'] = events_np['timestamp'] * 1
            events['p'] = events_np['polarity']
            events['x'] = events_np['x']
            events['y'] = events_np['y']
        else:
    ...
    def _stop(self):
        self._t_start_rec = self.rec_events[0]['t'][0]

        len_ev = 0
        for i in range(len(self.rec_events)):
            self.rec_events[i]['t'] = self.rec_events[i]['t'] - self._t_start_rec
            len_ev += len(self.rec_events[i])
        
        events = np.zeros(len_ev, dtype=np.dtype([("y", int), ("x", int), ("p", int), ("t", np.int64)]))
        events['t'] = np.concatenate([e['t'] for e in self.rec_events])
        events['p'] = np.concatenate([e['p'] for e in self.rec_events])
        events['x'] = np.concatenate([e['x'] for e in self.rec_events])
        events['y'] = np.concatenate([e['y'] for e in self.rec_events])

        self.dat_writer.write(events)


        self.dat_writer.close()
        super()._stop()
